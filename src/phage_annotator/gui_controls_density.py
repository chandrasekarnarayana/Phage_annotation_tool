...
class DensityControlsMixin:
    ...
    if checked and self._density_last_result is not None:
    self._density_overlay = self._density_last_result.density_map
    self._density_overlay_extent = (0, self._density_overlay.shape[1], self._density_overlay.shape[0], 0)
    else:
    self._density_overlay = None
    self._refresh_image()
    ...
    return
    path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save density map", "", "TIFF (*.tif);;NPY (*.npy)")
    if not path:
    return
    ...
    tif.imwrite(path, self._density_last_result.density_map.astype(np.float32, copy=False))
