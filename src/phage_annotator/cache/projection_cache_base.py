"""Base data structures and cache management for ProjectionCache."""
...
@dataclass
class CacheItem:
    """Single cached array with its accounted byte size."""
...
class CacheTelemetry:
    """Cache hit/miss and eviction telemetry counters."""
...
class ProjectionCacheBase:
    """Initialization and CRUD operations for ProjectionCache."""
    ...
    def get_lazy(self, key: CacheKey) -> Optional[CompressedBuffer]:
        ...
        if buffer is not None:
            self._telemetry.hits += 1
            self._telemetry.hits_this_cycle += 1
            logger.debug(f"[P7b] Lazy disk cache hit for {key}, ratio={buffer.get_compression_ratio():.2f}x")
            return buffer
