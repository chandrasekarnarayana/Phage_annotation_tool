...
class SmlmControlsMixin:
    ...
    self._append_log(f"[SMLM] ThunderSTORM job={self._smlm_job_id} frames={frames} detections={len(locs)}")
    self._refresh_image()
    ...
    handle = self.jobs.submit(_job, name="SMLM (ROI)", on_result=_on_result, on_error=_on_error, on_progress=_on_progress)
    ...
    writer = csv.writer(f)
    writer.writerow(["frame_index", "x_px", "y_px", "sigma_px", "photons", "background", "uncertainty_px", "label"])
    ...
    if not path:
    return
    data = np.zeros((len(self._smlm_results),), dtype=[
    ("frame_index", "i4"),
    ("x_px", "f4"),
    ("y_px", "f4"),
    ("sigma_px", "f4"),
    ...
    if self.smlm_panel is None:
    return
    path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select Deep-STORM model", "", "Model Files (*.pt)")
    if path:
    ...
    self._append_log(f"[SMLM] Deep-STORM job={self._deepstorm_job_id} frames={frames} detections={len(locs)}")
    self._refresh_image()
    ...
    handle = self.jobs.submit(_job, name="Deep-STORM (ROI)", on_result=_on_result, on_error=_on_error, on_progress=_on_progress)
    ...
    return
    path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Export SR Image", "", "TIFF Files (*.tif);;PNG Files (*.png)")
    if not path:
    return
    ...
    thunder.max_candidates_spin.setValue(int(params.get("max_candidates_per_frame", thunder.max_candidates_spin.value())))
