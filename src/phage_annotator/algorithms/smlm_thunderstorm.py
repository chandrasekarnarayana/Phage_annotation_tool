...
from phage_annotator.algorithms.analysis import fit_gaussian_2d, local_maxima, mad_sigma
...
def detect_candidates(
    ...
    if roi_mask is not None:
        # Replace masked values with -inf without copying the entire array
        masked = np.where(roi_mask, filtered, -np.inf)
    else:
        masked = filtered
