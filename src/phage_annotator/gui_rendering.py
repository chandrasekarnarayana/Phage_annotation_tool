...
class RenderingMixin:
    ...
    def _refresh_image(self) -> None:
        ...
        if crop_rect:
            slice_data = self._apply_crop_rect(slice_data, crop_rect, (slice_data.shape[0], slice_data.shape[1]))
            if support_slice is not None:
                support_slice = self._apply_crop_rect(
                    support_slice, crop_rect, (support_slice.shape[0], support_slice.shape[1])
                )
        ...
        if self._interactive:
            ...
            if level > 0:
                ...
                slice_display = self._get_pyramid_display(prim.id, "frame", slice_data, t_idx, z_idx, self.crop_rect or (0, 0, 0, 0), level)
                mean_display = self._get_pyramid_display(prim.id, "mean", mean_data, -1, -1, self.crop_rect or (0, 0, 0, 0), level)
                std_display = self._get_pyramid_display(prim.id, "std", std_data, -1, -1, self.crop_rect or (0, 0, 0, 0), level)
                comp_display = self._get_pyramid_display(prim.id, "composite", comp_data, -1, -1, self.crop_rect or (0, 0, 0, 0), level)
                if support_slice is not None:
                    support_display = self._get_pyramid_display(
                        supp.id, "support", support_slice, t_idx, z_idx, self.crop_rect or (0, 0, 0, 0), level
                    )
            else:
        ...
        if self.show_sr_overlay:
            ...
            overlay_extent = self._sr_overlay_extent if self._sr_overlay is not None else self._smlm_overlay_extent
        ...
        if self.scale_bar_enabled:
            ...
            if cal.pixel_size_um_per_px:
                ...
                extent = extents.get("frame") or (0, slice_display.shape[1], slice_display.shape[0], 0)
        ...
        ctx = RenderContext(
            ...
            projections={"mean": mean_display, "std": std_display, "composite": comp_display},
            ...
            threshold_mask=self._threshold_preview_mask if self._threshold_preview_mask is not None else None,
            ...
            overlay_alpha=float(self._density_overlay_alpha) if self._density_overlay is not None else 0.6,
            overlay_norm=None,
            overlay_cmap=plt.get_cmap(self._density_overlay_cmap) if self._density_overlay is not None else None,
        ...
        if self.ax_mean is not None and not mean_ready:
            self.ax_mean.text(0.5, 0.5, "Computing mean...", transform=self.ax_mean.transAxes, ha="center", va="center")
        if self.ax_comp is not None and not comp_ready:
            self.ax_comp.text(
                0.5, 0.5, "Computing composite...", transform=self.ax_comp.transAxes, ha="center", va="center"
            )
        if self.ax_std is not None and not std_ready:
            self.ax_std.text(0.5, 0.5, "Computing std...", transform=self.ax_std.transAxes, ha="center", va="center")
    ...
    def _build_roi_overlays(self) -> Dict[str, List[Tuple[str, object, str]]]:
        overlays: Dict[str, List[Tuple[str, object, str]]] = {panel: [] for panel in ["frame", "mean", "composite", "support"]}
    ...
    def _get_display_mapping(self, image_id: int, panel: str, data: Optional[np.ndarray]) -> DisplayMapping:
    ...
    def _draw_diagnostics(self, slice_data: np.ndarray, vmin: float, vmax: float) -> None:
        ...
        if self.profile_enabled and self.profile_chk.isChecked() and profile_visible and self.ax_line is not None:
            self.ax_line.clear()
        ...
        if self.hist_enabled and self.hist_chk.isChecked() and hist_visible and self.ax_hist is not None:
    ...
    def _hist_values(self, slice_data: np.ndarray) -> Optional[np.ndarray]:
        ...
        if scope == "Sampled stack":
            cache_key = (self.primary_image.id, region, self.crop_rect, self.roi_rect, self.roi_shape)
    ...
    def _hist_values_current(self, slice_data: np.ndarray) -> np.ndarray:
        if self.hist_region == "crop" and self.crop_rect is not None:
            data = self._apply_crop_rect(slice_data, self.crop_rect, (slice_data.shape[0], slice_data.shape[1]))
        else:
            data = slice_data
    ...
    def _request_hist_job(self, cache_key) -> None:
            ...
            for t in range(0, t_count, t_step):
                for z in range(0, z_count, z_step):
                    ...
                    if region == "crop" and crop_rect is not None:
                        frame = self._apply_crop_rect(frame, crop_rect, (frame.shape[0], frame.shape[1]))
                    if region == "roi":
        ...
        handle = self.jobs.submit(_job, name="Histogram sample", on_result=_on_result, on_error=_on_error)
        self._hist_job_id = handle.job_id

    def _norm_cached(self, panel: str, mapping: DisplayMapping):
        key = (panel, float(mapping.min_val), float(mapping.max_val), float(mapping.gamma), mapping.mode)
