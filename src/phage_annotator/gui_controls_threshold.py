...
from phage_annotator.thresholding import AUTO_METHODS, PostprocessOptions, compute_threshold, make_mask, postprocess_mask, smooth_image
...
class ThresholdControlsMixin:
    ...
    "threshold_auto",
    {"method": values.method, "value": float(result), "region_roi": values.region_roi, "scope": values.scope},
    )
    self._threshold_refresh_preview()
    ...
    def _threshold_percentile_bounds(self, data: np.ndarray, values, roi_mask: Optional[np.ndarray]) -> Tuple[float, float]:
    if roi_mask is not None:
    sample = data[roi_mask]
    else:
    sample = data.ravel()
    ...
    sample = data
    if roi_mask is not None:
    sample = sample[roi_mask]
    return sample.ravel()
    # Sampled stack: pick frames and sample pixels
    prim = self.primary_image
    if prim.array is None:
    return data.ravel()
    ...
    def _threshold_to_full_mask(self, mask: np.ndarray, crop_offset: Tuple[int, int], full_shape: Tuple[int, int]) -> np.ndarray:
    ...
    y0, y1 = ys.min(), ys.max()
    self.roi_shape = "box"
    self.roi_rect = (float(x0), float(y0), float(x1 - x0), float(y1 - y0))
    ...
    return
    crop_offset = self._threshold_crop_offset(image.shape)
    self._threshold_mask_full = self._threshold_to_full_mask(self._threshold_preview_mask, crop_offset, image.shape)
    ...
    if values.scope == "Current slice":
    if mask_entry.get("t") != int(self.t_slider.value()) or mask_entry.get("z") != int(self.z_slider.value()):
    ...
    # All frames: recompute masks using stored threshold settings.
    ...
    if region_roi:
    if crop_rect:
    roi_local = (roi_rect[0] - crop_off_x, roi_rect[1] - crop_off_y, roi_rect[2], roi_rect[3])
    else:
    roi_local = roi_rect
    roi_mask = roi_mask_for_shape(frame.shape, roi_local, roi_shape)
    if settings.get("method") == "Manual":
    low = float(np.percentile(frame[roi_mask] if roi_mask is not None else frame.ravel(), settings.get("manual_low_pct", 20)))
    high = float(np.percentile(frame[roi_mask] if roi_mask is not None else frame.ravel(), settings.get("manual_high_pct", 99)))
    mask = make_mask(frame, low, high, invert=bool(settings.get("invert_mask", False)))
    else:
    thr = compute_threshold(frame[roi_mask] if roi_mask is not None else frame.ravel(), settings.get("method", "Otsu"), background=settings.get("background", "dark"))
    ...
    if show_boxes:
    x, y, w, h = p.bbox
    self._particles_overlays.append(("box", ((x - off_x) / scale, (y - off_y) / scale, w / scale, h / scale), color, selected))
    if show_ellipse:
    x, y, w, h = p.bbox
    self._particles_overlays.append(("ellipse", ((x - off_x) / scale, (y - off_y) / scale, w / scale, h / scale), color, selected))
    self._refresh_image()
    ...
    writer = csv.writer(f)
    writer.writerow(["frame", "area_px", "perimeter_px", "circularity", "centroid_x", "centroid_y", "eq_diameter", "bbox"])
    ...
    def _apply_threshold_settings(self, settings: dict) -> None:
    if self.threshold_panel is None:
    return
    tp = self.threshold_panel
    tp.target_combo.setCurrentText(str(settings.get("target", tp.target_combo.currentText())))
    tp.region_chk.setChecked(bool(settings.get("region_roi", tp.region_chk.isChecked())))
    tp.scope_combo.setCurrentText(str(settings.get("scope", tp.scope_combo.currentText())))
    tp.sample_spin.setValue(int(settings.get("sample_count", tp.sample_spin.value())))
    tp.method_combo.setCurrentText(str(settings.get("method", tp.method_combo.currentText())))
    tp.low_slider.setValue(int(settings.get("manual_low_pct", tp.low_slider.value())))
    tp.high_slider.setValue(int(settings.get("manual_high_pct", tp.high_slider.value())))
    tp.invert_chk.setChecked(bool(settings.get("invert_mask", tp.invert_chk.isChecked())))
    tp.background_combo.setCurrentText(str(settings.get("background", tp.background_combo.currentText())))
    tp.smooth_spin.setValue(float(settings.get("smooth_sigma", tp.smooth_spin.value())))
    tp.preview_chk.setChecked(bool(settings.get("preview", tp.preview_chk.isChecked())))
    tp.min_area_spin.setValue(int(settings.get("min_area_px", tp.min_area_spin.value())))
    tp.fill_holes_chk.setChecked(bool(settings.get("fill_holes", tp.fill_holes_chk.isChecked())))
    tp.open_spin.setValue(int(settings.get("open_radius_px", tp.open_spin.value())))
    tp.close_spin.setValue(int(settings.get("close_radius_px", tp.close_spin.value())))
    tp.despeckle_chk.setChecked(bool(settings.get("despeckle", tp.despeckle_chk.isChecked())))
