"""Analysis qc composite impl helpers for the phage annotation tool.
...
from phage_annotator.analysis.qc_issue_types import (
    IssueSeverity, QCIssue, DuplicateValidator, OutOfBoundsValidator, MissingLabelValidator,
)
from phage_annotator.analysis.qc_density_validators import DensityClusterValidator, PoissonConsistencyValidator
...
class QCValidator:
    ...
    def validate(
        ...
        """Run all QC checks and return issues sorted by severity."""
        ...
        if thresholds.enabled_duplicate_check:
            all_issues.extend(
                DuplicateValidator.find_duplicates(annotations, image_id=image_id, threshold=dup_threshold)
            )

        if thresholds.enabled_bounds_check:
            all_issues.extend(
                OutOfBoundsValidator.find_out_of_bounds(
                    annotations, image_id=image_id, image_shape=image_shape, safety_margin=oob_margin)
            )

        if thresholds.enabled_label_check:
            all_issues.extend(
                MissingLabelValidator.find_missing_labels(
                    annotations, image_id=image_id, allowed_labels=allowed_labels)
            )

        if thresholds.enabled_density_check:
            all_issues.extend(
                DensityClusterValidator.find_high_density_clusters(
                    annotations, image_id=image_id, image_shape=image_shape,
                    grid_size=d_grid_size, min_density=d_min_count)
            )
        ...
        if (thresholds.enabled_illumination_check or thresholds.enabled_photobleaching_check or
            ...
            all_issues.extend(
                ImageArtifactValidator.find_artifacts(image_array=image_array, image_id=image_id)
            )

        if thresholds.enabled_image_fano_check:
            all_issues.extend(
                ImageArtifactValidator.find_image_signal_stochasticity(image_array=image_array, image_id=image_id)
            )

        if thresholds.enabled_annotation_fano_check:
            all_issues.extend(
                ImageArtifactValidator.find_annotation_stochasticity(
                    annotations=annotations, image_id=image_id, image_shape=image_shape)
            )
