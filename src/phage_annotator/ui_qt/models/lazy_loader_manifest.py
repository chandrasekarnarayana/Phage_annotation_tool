"""Models lazy loader manifest helpers for the phage annotation tool.
...
class LazyLoaderManifest:
    ...
    def _append_rows(self, path: str, rows: List[dict], depth: int) -> None:
        """Append rows for the current workflow."""
    ...
    def _child_paths(self, parent_path: str) -> List[str]:
        """Handle the child paths helper flow."""
        children = [
            entry.path
    ...
    def _add_root(
        ...
        """Add root for the current workflow."""
        root = Path(root)
        if root.is_dir():
    ...
    def _ensure_parent_dirs(self, file_path: Path, root: Path) -> None:
        """Ensure parent dirs for the current workflow."""
    ...
    def _collect_subtree(self, path: str) -> List[str]:
        """Collect subtree for the current workflow."""
        rows = [path]
