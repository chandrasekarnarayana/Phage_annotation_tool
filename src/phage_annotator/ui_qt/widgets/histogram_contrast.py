...
class HistogramContrastMixin:
    """Method group 1 extracted from ContrastDialog."""
    ...
    def _build_ui(self, vmin: float, vmax: float) -> None:
        """Build ui for the current workflow."""
    ...
    def _compute_histogram(self) -> None:
        """Compute histogram for the current workflow."""
    ...
    def _render_histogram(self) -> None:
        """Render histogram for the current workflow."""
        self._ax.clear()
    ...
    def _update_vlines(self) -> None:
        """Update vlines for the current workflow."""
    ...
    def _auto_adjust(self) -> None:
        """Handle the auto adjust helper flow."""
        if self._data.size == 0:
            return
        ...
        class _AutoWorker(QtCore.QObject):
            ...
            def _compute(self) -> Tuple[float, float]:
                """Compute compute for the current workflow."""
    ...
    def _compute_auto_range(self, method: str, low: float, high: float) -> Tuple[float, float]:
        """Compute auto range for the current workflow."""
