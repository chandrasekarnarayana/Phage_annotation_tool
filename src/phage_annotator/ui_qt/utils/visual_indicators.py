...
This module provides Qt widgets that display visual indicators for various
application states, including modality selection, synchronization status,
...
class ModalityIndicator(QtWidgets.QWidget):
    ...
    Displays current modality with custom name, projection type,
    and synchronization state.
    """

    def __init__(self, parent: QtWidgets.QWidget | None = None):
        """Initialize modality indicator.
    ...
    def set_modality_info(
        ...
        """Update modality indicator information.
        
        Args:
            name: Modality display name
            projection_type: Projection type (Raw, Mean, Std, Min, Max)
            is_active: Whether this modality is currently active
            is_linked: Whether this modality is linked for synchronization
        """
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:
        ...
        # Link indicator
        if self._is_linked:
            painter.fillRect(QtCore.QRect(self.width() - 18, 5, 13, 13),
                            QtGui.QColor("#4c78a8"))
...
class SyncStateIndicator(QtWidgets.QWidget):
    ...
    Shows which display properties are synchronized across modalities.
    """

    def __init__(self, parent: QtWidgets.QWidget | None = None):
        """Initialize sync state indicator.
    ...
    def set_sync_state(
        ...
        Args:
            vmin: Whether vmin is synchronized
            vmax: Whether vmax is synchronized
            contrast: Whether contrast is synchronized
            brightness: Whether brightness is synchronized
        """
        self._sync_vmin = vmin
        self._sync_vmax = vmax
        self._sync_contrast = contrast
        self._sync_brightness = brightness
        self.update()

    def paintEvent(self, event: QtGui.QPaintEvent) -> None:
        """Paint sync state indicators."""
        painter = QtGui.QPainter(self)
        painter.setRenderHint(QtGui.QPainter.RenderHint.Antialiasing, True)
        ...
        x_pos = 45
        sync_items = [
            ("vmin", self._sync_vmin),
            ("vmax", self._sync_vmax),
            ("C", self._sync_contrast),
        ]
        ...
        for i, (label, is_synced) in enumerate(sync_items):
            ...
            font.setBold(False)
            painter.setFont(font)
            painter.drawText(QtCore.QRect(x_pos + i * 35 - 2, 27, 20, 9),
                           QtCore.Qt.AlignmentFlag.AlignCenter, label)
...
class DisplaySettingsBadge(QtWidgets.QWidget):
    ...
    Displays mode indicator (e.g., "Auto" for auto-contrast, "Log" for log scale).
    """

    def __init__(self, parent: QtWidgets.QWidget | None = None):
        """Initialize display settings badge.
        
        Args:
            parent: Parent widget
        """
        super().__init__(parent)
        self._mode = "Auto"  # Auto, Linear, Log, Sqrt
    ...
    def set_display_mode(self, mode: str, is_modified: bool = False) -> None:
        ...
        Args:
            mode: Display mode label (Auto, Linear, Log, Sqrt)
            is_modified: Whether settings have been modified from defaults
        """
        self._mode = mode
...
class StatusIndicatorBar(QtWidgets.QWidget):
    """Compact status bar showing all indicators at once."""
    ...
    def update_status(
        ...
        Args:
            modality_name: Current modality name
            projection_type: Current projection type
            is_active: Whether modality is active
            is_linked: Whether modality is linked
            display_mode: Current display mode
            is_modified: Whether settings modified from defaults
            sync_vmin: Whether vmin is synchronized
            sync_vmax: Whether vmax is synchronized
            sync_contrast: Whether contrast is synchronized
        """
        self.modality_indicator.set_modality_info(
            modality_name, projection_type, is_active, is_linked
        )
